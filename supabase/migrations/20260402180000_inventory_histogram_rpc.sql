-- Histogram for inventory popover (pre–stock filter), computed in DB for large catalogs.
...
GRANT EXECUTE ON FUNCTION public.inventory_stock_histogram(uuid, text, timestamptz, text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.inventory_stock_histogram(uuid, text, timestamptz, text) TO service_role;
