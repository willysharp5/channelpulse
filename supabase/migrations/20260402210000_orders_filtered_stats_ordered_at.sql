-- Extend orders_filtered_stats with optional ordered_at lower bound (list KPIs + date filter).
DROP FUNCTION IF EXISTS public.orders_filtered_stats(uuid, text, text, text);
...
GRANT EXECUTE ON FUNCTION public.orders_filtered_stats(uuid, text, text, text, timestamptz) TO authenticated;
GRANT EXECUTE ON FUNCTION public.orders_filtered_stats(uuid, text, text, text, timestamptz) TO service_role;
